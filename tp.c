...
int main()
{
    ...
    {
        printf("Cantidad de choferes inválida.\n");
        return 1;
    }
    ...
    do
    {
        ...
        printf("2. Mostrar lista de choferes ordenada por cantidad de kilómetros conducidos en la semana\n");
        printf("3. Mostrar lista de choferes con su salario semanal\n");
        printf("4. Buscar por Chofer\n");
        printf("5. Mostrar lista de choferes con promedio de kilómetros conducidos en la semana\n");
        printf("6. Mostrar estadística de choferes\n");
        printf("7. Salir\n");
...
{
    int len = strlen(cuil);
    if (len != 8) // El CUIL debe tener exactamente 11 caracteres
        return 0;
...
void cargar_choferes(Chofer choferes[MAX_CHOFERES], int n)
{
    for (int i = 0; i < n; i++)
    {
        ...
        {
            printf("CUIL inválido. Ingrese nuevamente el CUIL del chofer %d: ", i + 1);
            scanf("%s", choferes[i].cuil);
        }
        ...
        for (int j = 0; j < 5; j++)
        {
            ...
            printf("Ingrese los kilómetros conducidos el %s del chofer %d: ", dias[j], i + 1);
...
void mostrar_lista_choferes_ordenada_kilometros(Chofer choferes[MAX_CHOFERES], int n)
{
    ...
    printf("Apellido y Nombre - Total de kilómetros conducidos en la semana\n");
    for (int i = 0; i < n; i++)
    {
...
{
    float precio_por_km;
    printf("Ingrese el precio por kilómetro conducido: ");
...
void buscar_chofer(Chofer choferes[MAX_CHOFERES], int n)
{
    ...
    {
        printf("CUIL inválido. Ingrese nuevamente el CUIL del chofer a buscar: ");
        scanf("%s", cuil);
    }
    ...
    for (int i = 0; i < n; i++)
    {
        if (strcmp(choferes[i].cuil, cuil) == 0)
        {
            ...
            printf("Apellido: %s\n", choferes[i].apellido);
            printf("Nombre: %s\n", choferes[i].nombre);
            printf("Kilómetros conducidos por día: Lunes %d, Martes %d, Miércoles %d, Jueves %d, Viernes %d\n",
                   choferes[i].km[0], choferes[i].km[1], choferes[i].km[2], choferes[i].km[3], choferes[i].km[4]);
            printf("Total de kilómetros conducidos en la semana: %d\n", total_km_semanal(choferes[i].km));
            return;
        }
    }
...
void mostrar_promedio_kilometros(Chofer choferes[MAX_CHOFERES], int n)
{
    printf("CUIL - Apellido y Nombre - Promedio de kilómetros conducidos en la semana\n");
    for (int i = 0; i < n; i++)
    {
...
void mostrar_estadisticas(Chofer choferes[MAX_CHOFERES], int n)
{
    ...
    printf("El chofer con mayor cantidad de kilómetros conducidos en la semana es %s %s con %d km\n", max_km_chofer->apellido, max_km_chofer->nombre, max_km);
    printf("El chofer con menor cantidad de kilómetros conducidos en la semana es %s %s con %d km\n", min_km_chofer->apellido, min_km_chofer->nombre, min_km);
    printf("El chofer con mayor promedio de kilómetros conducidos en la semana es %s %s con %.2f km\n", max_prom_km_chofer->apellido, max_prom_km_chofer->nombre, max_prom_km);
    printf("El chofer con menor promedio de kilómetros conducidos en la semana es %s %s con %.2f km\n", min_prom_km_chofer->apellido, min_prom_km_chofer->nombre, min_prom_km);
    printf("El promedio total de kilómetros conducidos por todos los choferes es %.2f km\n", (float)total_km / n);
}
